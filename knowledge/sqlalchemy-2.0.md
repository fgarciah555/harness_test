...
**Verificado:** 2026-08-26, `sqlalchemy==2.0.35`, real (no mocks) —
rompía el login completo de `Web_coas/web-portal-coas-destino`.
...
**Encontrado en:** `DAL-AUTH-002`, 2026-08-26.
...
reales para el mismo `ca_rut` devolvía 1 sola, sin excepción.

Una tabla legada sin PK real a veces tiene una columna que "suena" a
identidad (`ca_rut`) pero no es única en el uso real (1 fila por comercio
de acceso). Declararla `primary_key=True` no falla en `INSERT`/`CREATE
TABLE` — el identity map de SQLAlchemy trata las filas con el mismo valor
de PK como la misma entidad y devuelve solo la primera cargada.
...
**Encontrado en:** `DAL-AUTH-001` (`Usuario.ca_rut` sola → compuesta
`(ca_rut, ca_cod)`), 2026-08-26 — ver también `backend-architecture.md`,
...
```python
.where(tabla.c.ca_fec // 100 == anio_mes_int)   # OK: entera
.where(tabla.c.ca_fec / 100 == anio_mes_int)    # MAL: real, sin importar el tipo de la columna
```
...
**Encontrado en:** `DAL-DEPOSITO-001` (`deposito_repository.py`, 3
ocurrencias, alimentaba también el cálculo de saldo), 2026-08-26.

## `Base.metadata.create_all()` NO altera tablas que ya existen — solo crea las que faltan

**Verificado:** 2026-08-25, `sqlalchemy==2.0.35` (`reportePruebaStress/requirements.txt`).

**Contexto:** en proyectos sin Alembic/migraciones (varios de los que toca
este harness arrancan `Base.metadata.create_all(bind=engine)` al boot, ver
`app/main.py` de `reportePruebaStress`), es fácil asumir que agregar una
columna nueva a una entidad declarativa (`Mapped[str]` nuevo en una clase
`Base` existente) alcanza para que la próxima corrida de la app la tenga
disponible en la base real. **No es así**: `create_all()` compara qué
*tablas* faltan contra el `MetaData` y crea esas — nunca hace `ALTER TABLE`
sobre una tabla que ya existe, aunque su definición en Python haya cambiado.
...
**Encontrado en:** `reportePruebaStress`, item `RPT-001` (revisión 5),
2026-08-25 — `TestEntity` ganó columnas `proyecto`/`ambiente`; el código
generado y aprobado por Compliance rompió `GET /reportes/getTestList` y
`POST /reportes/registraPrueba` contra la base real (3 filas ya existentes
en `test`) hasta correr el `ALTER TABLE` de arriba.
...
`checks/format_check.py`), `reportePruebaStress`.
...
**Mismo síntoma, causa distinta, ya visto en el mismo proyecto:** un import
...
**Encontrado en:** `reportePruebaStress`, items `RPT-003` (imports de
entidades en `main.py`, 2026-08-24) y `RPT-002`/`PDF-001` (import muerto de
`ReqReporte`/`datetime`, mismo día) — ambos rechazados por
`format_check.py` en el primer intento, cero costo de LLM porque el
rechazo es determinístico y previo a Compliance.

## `db.execute(select(Entidad)).first()` NO devuelve la instancia de la entidad

**Verificado:** 2026-08-21, `sqlalchemy>=2.0` (versión exacta resuelta en
`web-portal-coas-migrado/backend/requirements.txt`).
...
**Encontrado en:** `web-portal-coas-migrado`, `COAS-AUTH-002`, 2026-08-21 —
no lo atrapó ningún test hasta que se especificó explícito en
`detalle_tecnico`; el bug es silencioso (no rompe en `pytest` con dobles/
monkeypatch, solo rompería contra una base real) — ver también la entrada de
abajo sobre queries con `JOIN`, que si usan `select()`/`.mappings()` con
columnas explícitas evitan esta ambigüedad de raíz.
...
tabla del `JOIN` (pasó de verdad: se pidió `VentaDetalleLocal.vca_glosa`
cuando `vca_glosa` solo existe en `Local`, la tabla joineada).
...
consumidor adivinó mal las claves del dict (`vca_mto` vs `vca_monto` según
la función, `vca_est`/`vca_nuc` en vez de `vca_edo`/`vca_ncuo`, hasta una
clave inventada `vca_fecha` que nunca existió) — errores silenciosos
(`KeyError` en runtime, invisibles para `format_check.py` porque no es un
import roto) que Compliance aprobó falsamente más de una vez porque revisa
código, no ejecuta contra datos reales. La causa de fondo: un `dict` no es
un contrato — sus claves no aparecen en ningún lado que la `interfaz` del
item pueda declarar de forma verificable.
...
`ventadetalleloc.vca_monto` es `numeric(19,4)` (→ `Decimal`), pero
`ventadetalle.vca_mto` es `int4` (→ `int`) — dos columnas con nombres
parecidos, de tablas distintas, con tipos reales distintos. Generalizar
("todos los 'monto' son Decimal") produce el error tan fácil como asumir
que todos son `int`.
...
**Encontrado en:** `web-portal-coas-migrado`, `COAS-REP-003`, 2026-08-24 —
además, un intento de arreglar esto a mano (edición directa de
`detalle_tecnico` sin re-verificar contra el DDL) generalizó mal la regla
en sentido inverso (marcó `vca_mto`/`int` como si fuera `vca_monto`/
`Decimal`) y produjo el mismo tipo de bug por el motivo opuesto — la
lección aplica tanto al modelo como a quien edita el plan a mano.

**Encontrado en:** `web-portal-coas-migrado`, `COAS-REP-002`/`COAS-REP-003`,
2026-08-21 — el patrón `list[dict]` fue el origen de 3 rechazos oscilando en
`COAS-REP-002` (resuelto por `executor_senior`) y del caso de oscilación más
severo visto hasta ahora en `COAS-REP-003` (3 rechazos seguidos, cada
reintento perdía un fix anterior al corregir otro; `executor_senior`
necesitó una tabla de claves verificadas a mano para resolverlo). El bug de
`COALESCE` se coló sin que `format_check.py` (no revisa librerías externas)
ni Compliance lo atraparan — recién lo destapó el smoke test de otro item
(`COAS-CORE-003`) al importar la cadena completa de la app. Ver `handoff.md`,
sección "Quinta prueba real", para el detalle completo.
